...
def eval_all(expressions, env):
    ...
    return scheme_eval(do_quote_form(expressions.second, env), env, True)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            for symbol in range(len(vals)):
                frame.bindings[formals[symbol]] = vals[symbol]
        else:
            raise SchemeError(" too many or too few vals ")
        return frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        env.define(target.first, scheme_eval((Pair("lambda", Pair(target.second, expressions.second))), env))
        return target.first
    else:
...
def do_if_form(expressions, env):
    ...
    if scheme_eval(expressions.first, env) is False:
        if len(expressions) == 2:
            ...
            return scheme_eval(expressions.second[1], env, True)
    else:
        return scheme_eval(expressions.second[0], env, True)
...
def do_and_form(expressions, env):
    ...
    while exp is not nil:
        last = scheme_eval(exp.first, env) 
        if not last:
            return False 
        exp = exp.second
    return last
...
def do_or_form(expressions, env):
    ...
    exp = expressions
    last = False
    while exp is not nil and last is False:
        last = scheme_eval(exp.first, env) 
        if last:
            return last
        exp = exp.second  
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    newframe = env.make_child_frame(nil, nil)
    for element in bindings:
        if scheme_symbolp(element.first):
            check_form(element, min = 2, max = 2)
            newframe.define(element.first, scheme_eval(element[1], env))
        else:
            raise SchemeError
    return newframe
...
def check_formals(formals):
    ...
    arguments = []
    for x in formals:
        if (not scheme_symbolp(x)) or (x in arguments):
            raise SchemeError("list of formals is not a well-formed list of symbols or symbol is repeated")
        arguments.append(x)
